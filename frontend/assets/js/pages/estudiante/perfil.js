    ...
    async function inicializarModulo() {
        ...
        async function manejarEliminarCuenta() {
            ...
            try {
                ...
                if (response.success) {
                    window.toastManager.success('Cuenta eliminada. Puedes reactivarla iniciando sesión en los próximos 30 días.');
                    guardarVentanaReactivacion();
                    setTimeout(() => {
                        window.Utils.clearStorage();
                        window.location.href = `${config.UI.RUTAS.LOGIN}?reactivar=1`;
                    }, 2000);
                } else {
        ...
        function guardarVentanaReactivacion() {
            const usuario = estado.datosPerfil?.usuario || estado.usuario;
